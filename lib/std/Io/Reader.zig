...
/// "Pump" exactly `n` bytes from the reader to the writer.
///
/// When draining `w`, ensures that at least `preserve` bytes remain
/// buffered.
pub fn streamExactPreserve(r: *Reader, w: *Writer, preserve: usize, n: usize) StreamError!void {
    const slice = try w.writableSlicePreserve(preserve, n);
    var fixed_writer: Writer = .fixed(slice);
    return r.streamExact(&fixed_writer, n);
}
